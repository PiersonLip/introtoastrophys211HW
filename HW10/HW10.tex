...
\[m = 1.4 M_\odot, \qquad r_{NS} = 10\text{km}\]
We find\dots
\[v_{rot} \approx 4.7 \times 10^{4}\text{m/s} \]
\[v_{crit} \approx 1.36 \times 10^{8} \text{m/s}\]

This means that a NS spinning with that rate \textit{does} make sense, this is because \( v_{rot} < v_{crit}\), meaning that the NS will \textit{not} break apart, and could have a pulse of 1.337s
...
\[a = 960 \text{AU}, \qquad M_{bh} = 7.22914 \times 10^{36} \text{kg} \]
We find 
